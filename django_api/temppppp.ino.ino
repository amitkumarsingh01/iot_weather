...
void sendDataToThingSpeak(float temperature, float pressure, float altitude, float sealevelPressure, float realAltitude, float humidity, int mqSensorValue1, int mqSensorValue2)
{
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        soilMoisture["mq2"] = mqSensorValue1;
        soilMoisture["mq3"] = mqSensorValue2;
        soilMoisture["pressure"] = pressure;
        soilMoisture["altitude"] = altitude;
        soilMoisture["sealevel_pressure"] = sealevelPressure;
        soilMoisture["real_altitude"] = realAltitude;
        ...
        if (httpResponseCode == 200 || httpResponseCode == 202 || httpResponseCode == 201)
        {
